...
function Login() {
  ...
  const{showsuccessmessage,setUsername,setImage}=useGlobalContext();
  const handleSubmit=async(e)=>{
      e.preventDefault();
      const response= await fetch("https://farmeasy-zwa7.onrender.com/api/loginuser",{
          method:"POST",
          headers:{
      ...
      if(json.success){
        ...
        localStorage.setItem("authToken",json.image)
        navigate("/");
      }
  }
  return (
  ...
  <div className="container py-5 h-100">
    <div className="row d-flex align-items-center justify-content-center h-100">
      ...
      <img src={process.env.PUBLIC_URL + "/20230226_184157_0000.png"} className="img-fluid" alt="Phone image" />
      </div>
